...
export class BookingService {
  ...
  constructor() { }
  private bookings:Booking[] = [new Booking( 1, "Juan Pérez", "+34 600 123 456", "juan.perez@example.com", 4, "Prefiere mesa cerca de la ventana.", new
    ...
    new Booking( 2, "Ana Gómez", "+34 600 654 321", "ana.gomez@example.com", 2, "Celebración de aniversario.", new
      ...
      new Booking( 3, "Carlos López", "+34 600 789 012", "carlos.lopez@example.com", 6, "Requiere menú vegetariano.", new
        ...
        new Booking( 4, "María Fernández", "+34 600 345 678", "maria.fernandez@example.com", 3, "Traerá un pastel de cumpleaños"
    ...
    addNewBooking(booking: Booking) {
      this.bookings.push(booking); // Agregar tarea a la lista
    }

    addBooking(client: string, phone: string, email: string,persons:number,notes:string,date:Date): Booking {
      // Determinar el siguiente ID (autoincremento)
      const newId = this.bookings.length ? Math.max(...this.bookings.map(booking => booking.id)) + 1 : 1;
  

      let fechaCreacion = new Date(Date.now());
      let estadoBooking = BookingStatus.PENDING
      // Crear un nuevo usuario
      const newBooking = new Booking(newId, client, phone, email,persons,notes,date,fechaCreacion,estadoBooking);
  
      // Agregar el nuevo usuario al array
      this.bookings.push(newBooking);
  
      return newBooking;  // Retornamos la nueva persona
    }
    getPersonByID(id: number): Booking | null{
      for (let i = 0; i < this.bookings.length; i++) {
        if(id == this.bookings[i].id) {
